...
%   confl . information on conflict analysis when using local - global
%           trade-off.
%
%   Example:
...
p = zeros(nf,nx); ps = p;
sumDOF = sum([zeros(1,mf);f'])';           % sum of DOFs (zeros are added to keep column)
sumDOF = sumDOF(:,ones(1,nf));
NoRule = (sumDOF == 0);                    % no rule applicable
sumDOF = sumDOF + NoRule;                  % set zeros to one (avoid dividing through zero)
...
if flag == 0, 
    xx = f(:)./sumDOF(:);                  % Put everything in one column
    f1(:) = xx(:,ones(1,nx));              % reshape partition matrix: nx times
    x1 = f1.*x1;
...
elseif flag==2, % Algorithm 1 (Johansen&Babuska,2002)
    
    % Global LS 
    Psi=[];
    for i=1:nf,
      Psi=[Psi (f(:,i)*ones(1,nx)).*x'./sumDOF(:,ones(1,nx))];
    end;
    HG=Psi'*Psi/mx;
    fTG=-y'*Psi/mx;
    ...
    S=[];
    delta=[];
elseif flag ==3, % Algorithm 2 (Johansen&Babuska,2002)
     ...
     [popt_local,fval,exitflag,output,lambda]=quadprog(HL,fTL',A,b,Aeq,beq,lb,ub,p0,OPTIONS);
     
     % Optimize WLS solution globally within restrictions of deltath
     Psi=[];
     for i=1:nf,
       Psi=[Psi (f(:,i)*ones(1,nx)).*x'./sumDOF(:,ones(1,nx))];
     end;
     ...
     % Constraints on parameter vector + initializatiom

     ParamDelta=zeros(size(popt_local));
     for i=1:nf,
         ParamDelta(1+(i-1)*nx:i*nx)=deltath(i,:)';
     end;
     
     lb=popt_local-ParamDelta;  % Lower Boundary on estimated parameter
     ub=popt_local+ParamDelta;  % Upper Boundary on estimated parameter
     p0=popt_local;             % Initial parameter set
        
     OPTIONS=optimset('LargeScale','off','Display','off');

     [popt,fval,exitflag,output,lambda]=quadprog(HG,fTG',A,b,Aeq,beq,lb,ub,p0,OPTIONS);
     lambda_temp=lambda.upper-lambda.lower; % eq. (24) (Johansen&Babuska,2002)
     for i=1:nf,
       p(i,:)=(popt(1+(i-1)*nx:i*nx))';
       confl(i,:)=(lambda_temp(1+(i-1)*nx:i*nx))';
     end;
