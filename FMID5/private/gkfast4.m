function [f,v,P,V,D,J,M] = gkfast(x,f0,m,e,s,rho,gamma)
%adaption for cluster size
%
...
if (nargin < 7), gamma = 0;    elseif isempty(gamma),   gamma = 0; end;
...
l=0; %Parameter of Keller, (0 means switched off)
...
pt=eye(nx)*det(cov(x)).^(1/nx); %The added diagonal matrix
...
while  max(max(f0-f)) > e
   ...
   for j = 1 : c,                        % for all clusters
      ...
      if rcond(p)<1e-15;
         ...
         p=ev*ei*inv(ev);	
         % Calculate distances
         M = (prod(diag(ei))/rho(j))^inx*inv(p);
      else
         % Calculate distances
         M = (det(p)/rho(j))^inx*inv(p);
      end
      d(:,j) = sum((xv*M.*xv)')'; %the distances
   ...
   %change the cluster size (Keller)
   i=1; r=[];
   for i=1:c
      r(i) = sum(diag(ones(nx,1)*fm(:,i)'.*xv'*xv))^(1/(l+1));
   end
   r=r/sum(r);
   rho=(1./r).^l;
   ...
   % (makes the level plot, slow implementation)
   disp(['rho = ' num2str(rho)]);
   
   save tempfile
   
   FM.zmin{1}=min(x);   FM.zmax{1}=max(x);
   FM.m=m; FM.c=c; Dat.U=x(:,1); Dat.Y=x(:,2);
   Part.F{1}=P; Part.V{1}=v;
   cimage2   
   drawnow
   clear 
