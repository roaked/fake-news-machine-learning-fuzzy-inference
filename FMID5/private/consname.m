function cons = consname(FM)
% CONSNAME    extract names of consequent variables from FM.
%     CONS = CONSNAME(FM) is a cell array of cell arrays
%     containing the names of all consequent variables
...
for k = 1 : NO,
   ...
   for j = 1 : NO,
      if iscell(ny),
         for i = 1 : size(ny{2}{k,j},2),
            %for i = 1 : ny(k,j),
            vars{ii} = [outvar{j} '(k-' num2str(ny{2}{k,j}(1,i)) ')'];
      ...
      if iscell(nu),
         for i = 1 : size(nu{2}{k,j},2),
            ...
            if ~nu{2}{k,j}(1,i) 
               ...
               vars{ii} = [inpvar{j} '(k-' num2str(nu{2}{k,j}(1,i)) ')'];
            end   
