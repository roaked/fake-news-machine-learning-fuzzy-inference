...
%form the OUTPUT structure according output order
for i = na : -1 : 0,
  yr(:,i+1) = y(1+na-i:m-i);
end;

%form the INPUT structure according input order
for j = 1 : n-1  
  for i = nb(j)-1 : -1 : 0,
...
if nargout < 3,
  ...
  else  Y = yr(my-m+1:my,1); % model
     Yr = yr(my-m+1:my,2:na+1); %output regressors
     Ur = ur(mu-m+1:mu,:); %input regressors
  end
end;
